...
class SearchHistoryInline(admin.TabularInline):
    ...
    def view_results_link(self, obj):
        if obj.pk:
            url = reverse('admin:app_searchresult_changelist') + f'?history__id__exact={obj.pk}'
            return format_html('<a class="button" href="{}">Просмотреть результаты ({})</a>', 
                             url, obj.results_count)
        return "-"
...
class CrawledDataInline(admin.StackedInline):
    ...
    model = CrawledData
    fields = ['organization_name', 'http_status', 'has_phones', 'has_emails', 
              'has_addresses', 'view_details_link']
    readonly_fields = ['organization_name', 'http_status', 'has_phones', 
                      'has_emails', 'has_addresses', 'view_details_link']
    extra = 0
    max_num = 1
    can_delete = False
    ...
    def has_addresses(self, obj):
        count = obj.addresses.count() if obj.pk else 0
        return format_html('<span style="color: {};">{}</span>',
                         'green' if count > 0 else 'gray', f'📍 {count} адр.')
    has_addresses.short_description = 'Адреса'
...
class CrawledAddressInline(admin.TabularInline):
    """Инлайн для адресов в данных краулинга"""
    model = CrawledAddress
    fields = ['address_preview', 'address_cleaned', 'created_at']
    readonly_fields = ['address_preview', 'created_at']
    extra = 0
    ordering = ['-created_at']
    
    def address_preview(self, obj):
        if obj.address:
            return obj.address[:100] + '...' if len(obj.address) > 100 else obj.address
        return "-"
...
class SearchTopicAdmin(admin.ModelAdmin):
    ...
    def total_searches(self, obj):
        ...
        url = reverse('admin:app_searchhistory_changelist') + f'?topic__id__exact={obj.id}'
    ...
    actions = ['run_search_now', 'clone_topic']
    
    def run_search_now(self, request, queryset):
        for topic in queryset:
            # Здесь можно вызвать задачу для запуска поиска
            self.message_user(request, f'Запущен поиск для темы "{topic.name}"')
    run_search_now.short_description = '🚀 Запустить поиск сейчас'
    
    def clone_topic(self, request, queryset):
        for topic in queryset:
            topic.pk = None
            topic.name = f'{topic.name} (копия)'
            topic.save()
            self.message_user(request, f'Создана копия темы "{topic.name}"')
    clone_topic.short_description = '📋 Клонировать тему'
...
class SearchHistoryAdmin(admin.ModelAdmin):
    ...
    def topic_link(self, obj):
        url = reverse('admin:app_searchtopic_change', args=[obj.topic_id])
    ...
    def view_results(self, request, queryset):
        if queryset.count() == 1:
            ...
            url = reverse('admin:app_searchresult_changelist') + f'?history__id__exact={history.id}'
    ...
    def rerun_search(self, request, queryset):
        for history in queryset:
            # Здесь можно вызвать задачу для перезапуска поиска
            self.message_user(request, f'Перезапущен поиск для истории #{history.id}')
    rerun_search.short_description = '🔄 Перезапустить поиск'
...
class SearchResultAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'history_link', 'domain', 'title_preview', 
        'position', 'status_icons', 'created_at_short'
    ]
    list_display_links = ['id']
    list_filter = [
        'processed', 'skipped', 'created_at', 
        'history__topic__name', 'domain'
    ]
...
class CrawledDataAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'search_result_link', 'organization_name', 
        'http_status', 'data_summary', 'created_at_short'
    ]
    ...
    readonly_fields = ['created_at', 'updated_at', 'phones_count', 'emails_count', 'addresses_count']
    ...
    fieldsets = (
        ...
        ('Статистика', {
            'fields': ('phones_count', 'emails_count', 'addresses_count'),
        }),
        ('Системная информация', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
    ...
    def search_result_link(self, obj):
        url = reverse('admin:app_searchresult_change', args=[obj.search_result_id])
    ...
    def data_summary(self, obj):
        ...
        return format_html('<br>'.join(icons)) if icons else '❌ Нет данных'
...
class CrawledPhoneAdmin(admin.ModelAdmin):
    ...
    list_display = ['id', 'phone', 'crawled_link', 'phone_raw_short', 'created_at_short']
